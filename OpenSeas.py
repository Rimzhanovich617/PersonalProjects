...
pygame.display.set_caption("Open Seas")
pygame.font.init()
...
FPS = 60
VEL = 5
base_font = pygame.font.SysFont('Times New Roman', 32, False, False)
captain_text = 'Welcome landlubbers!'
character1_width, character1_height = 100, 100
character2_width, character2_height = 80, 80
text_box_color = (255, 255, 255)
input_rect = pygame.Rect(175,0,650,70)

SEA_Background_Image = pygame.image.load(os.path.join('open_seas_assets', 'sea.jpeg'))
SEA_Background = pygame.transform.scale(SEA_Background_Image, (WIDTH, HEIGHT))
NARWAL_Character_Image = pygame.image.load(os.path.join('open_seas_assets', 'narwal1.jpeg'))
NARWAL_Character = pygame.transform.scale(NARWAL_Character_Image, (character1_width, character1_height))
OCTOPUS_Character_Image = pygame.image.load(os.path.join('open_seas_assets', 'octopus1.jpeg'))
OCTOPUS_CHARACTER = pygame.transform.scale(OCTOPUS_Character_Image, (character1_width, character1_height))
PIRATE_CAPTAIN_Character_Image = pygame.image.load(os.path.join('open_seas_assets', 'pirate_bird.jpeg'))
PIRATE_CAPTAIN_Character = pygame.transform.scale(PIRATE_CAPTAIN_Character_Image, (character1_width, character1_height))
PIRATE_SHIP_IMAGE = pygame.image.load(os.path.join('open_seas_assets', 'pirate_ship.jpeg'))
PIRATE_SHIP_Character = pygame.transform.scale (PIRATE_SHIP_IMAGE, (character2_width, character2_height))

intro_text_1 = print("Welcome landlubbers!", "\n")
intro_text_2 = print("I'm Captain Longshivers and this is the prized possession of the ocean, the ship Persphone.")
intro_text_3 = print("Type your name and hit enter.")
all_captain_intro = [intro_text_1, intro_text_2, intro_text_3]


def draw_window(captain, ship):
    WIN.blit(PIRATE_CAPTAIN_Character, (captain.x, captain.y))
    WIN.blit(PIRATE_SHIP_Character, (ship.x, ship.y))
    pygame.display.update()

def captain_dialogue(position):
    font = pygame.font.SysFont('Times New Roman', 14, False, False)
    surface_1 = font.render("Hello Landlubbers!, "
                            "I'm Captain Longshivers and this is the prized possession of the ocean"
                            "the ship Persphone. Hit enter/return key", True, (0,0,0))
    WIN.blit(surface_1, input_rect)

def captain_question(position):
    font = pygame.font.SysFont('Times New Roman', 14, False, False)
    surface_2 = font.render("Type your name and hit enter", True, (0,0,0))
    WIN.blit(surface_2, input_rect)



def main():
    captain = pygame.Rect(50, 0, character1_width, character1_height)
    ship = pygame.Rect(850, 0, character2_width, character2_height)

    clock = pygame.time.Clock()
    run = True
    while run:
        WIN.blit(SEA_Background, (0,0))
        clock.tick(FPS)
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False

        pygame.draw.rect(WIN, text_box_color, input_rect)
        captain_dialogue(input_rect)
        keys_pressed = pygame.key.get_pressed()
        if keys_pressed[pygame.K_KP_ENTER]:
            quit()

        pygame.draw.rect(WIN, text_box_color, input_rect)
        captain_question(input_rect)

        pygame.display.flip()
        keys_pressed = pygame.key.get_pressed()
        if keys_pressed[pygame.K_a]:
            ship.x -= VEL
        draw_window(captain, ship)
        ...
        print(
            "Alright, now we can go in several directions: North, Northeast, East, Southeast, South, Southwest, West, and Northwest")
        ...
        while message != 'quit':
            ...
            close_narwal = ["NW21", "nw21", "NW 22", "nw22", "NW23", "nw23", "NW33", "nw33", "NW43", "nw43", "NW42",
                            "nw42",
                            "NW41", "nw41", "NW31", "nw31"]
            narwal_location = ["NW32", "nw32"]

            close_octopus = ["SE32", "se32", "SE33", "se33", "SE34", "se34", "SE44", "se44", "SE54", "se54", "SE53",
                             "se53",
                             "SE52", "se52", "SE42", "se42"]
